...
def load_data(filename, map_relations):
    ...
    for dialog in data:
        ...
        def cmp_relation(a, b):
            ...
            return 1
        dialog["relations"] = sorted(dialog["relations"], key=cmp_to_key(lambda a,b:cmp_relation(a, b)))
...
#    f_in.close()
    small_model = compress_fasttext.models.CompressedFastTextKeyedVectors.load(FLAGS.word_vector)
    embed = []
    cnt_pretrained = 0
    vocab_list_major = []
...
def preview_data(data, pred, map_relations, vocab, idx):
    with open('/home/vvs/Downloads/d_parse/chinadataset/out_ru_ft.txt', 'a') as out_file:
        map_relations_inv = {}
        ...
        for i, dialog in enumerate(data):
            ...
            for j, edu in enumerate(dialog["edus"]):
                print(j, edu["speaker"], ":", end=' ')
                out_file.write(f'{j} {edu["speaker"]} : ') 
                for token in edu["tokens"]:
                    if not token in vocab:
                        print("UNK(%s)" % token, end=' ')
                        out_file.write(f'UNK({token}) ') 
                    else:
                        print(token, end=' ')
            ...
            for relation in dialog["relations"]:
                ...
                out_file.write(f'{relation["x"]} {relation["y"]} {map_relations_inv[relation["type"]]}\n')
            print("predicted:")
            out_file.write(f'predicted: \n')
            for relation in pred[i]:
                print(relation[0], relation[1], map_relations_inv[relation[2]])
                out_file.write(f'{relation[0]} {relation[1]} {map_relations_inv[relation[2]]}\n')
...
def update_buffer(output_feed, input_feed, feed):
    return (output_feed + feed[0], dict(list(input_feed.items()) + list(feed[1].items())))    
...
def get_batches(data, batch_size, sort=True):
    ...
    batches = []
    for i in range(int(len(data) / batch_size + bool(len(data) % batch_size))):
